...
test("buildStandaloneDoc: bodyScripts を </article> の後に差し込む", () => {
  const doc = buildStandaloneDoc("t", "<p>x</p>", "", "<script>1</script>");
  assert.ok(doc.includes("</article><script>1</script></body>"));
  // 省略時は何も足さない(従来どおり)
  assert.ok(buildStandaloneDoc("t", "<p>x</p>", "").includes("</article></body>"));
});
